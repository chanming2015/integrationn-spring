...
public class SystemFunctionServiceImpl implements SystemFunctionService {
	...
	@Override
	public Result<Pager<SystemFunction>> pageSystemFunctions(SpecParam<SystemFunction> specs, Pager<SystemFunction> pager) {
		...
		// 返回结果
		PageObject<SystemFunction> pageObject = new PageObject<SystemFunction> ((int)resultSet.getTotalElements(), resultSet.getContent());
	...
	public Result<SystemFunction> updateSystemFunction(SystemFunction systemFunction) {
		...
		try{
			...
			systemFunctionFetched.setParentId(systemFunction.getParentId() == null ? systemFunctionFetched.getParentId() : systemFunction.getParentId());
			systemFunctionFetched.setLevel(systemFunction.getLevel() == null ? systemFunctionFetched.getLevel() : systemFunction.getLevel());
			systemFunctionFetched.setSerial(systemFunction.getSerial() == null ? systemFunctionFetched.getSerial() : systemFunction.getSerial());
			systemFunctionFetched.setName(systemFunction.getName() == null ? systemFunctionFetched.getName() : systemFunction.getName());
			systemFunctionFetched.setMenu(systemFunction.getMenu() == null ? systemFunctionFetched.getMenu() : systemFunction.getMenu());
			systemFunctionFetched.setActionUrl(systemFunction.getActionUrl() == null ? systemFunctionFetched.getActionUrl() : systemFunction.getActionUrl());
			// 持久对象
	 ...
	 */
	@Override
	public Result<List<SystemFunction>> listFunctions(
			SpecParam<SystemFunction> specs) {
		...
		try{
			...
			result = errorResult.newFailure("数据错误", "在获取功能列表时发生数据操作错误。");
			return result;
		}
		// 返回结果
