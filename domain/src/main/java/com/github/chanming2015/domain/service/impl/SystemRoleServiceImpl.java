...
public class SystemRoleServiceImpl implements SystemRoleService
{
    ...
    @Autowired
    private SystemRoleFunctionRepository systemRoleFunctionRepository;
    @Autowired
    ...
    @Override
    public Result<Pager<SystemRole>> pageSystemRoles(SpecParam<SystemRole> specs,
            Pager<SystemRole> pager)
    {
        ...
        // 返回结果
        PageObject<SystemRole> pageObject = new PageObject<SystemRole>(
    ...
    public Result<SystemRole> updateSystemRole(SystemRole systemRole)
    {
        ...
        try
        {
            ...
            systemRoleFetched.setName(systemRole.getName() == null ? systemRoleFetched.getName()
                    : systemRole.getName());
            systemRoleFetched
                    .setDescription(systemRole.getDescription() == null ? systemRoleFetched
                            .getDescription() : systemRole.getDescription());
            systemRoleFetched.setStatus(systemRole.getStatus() == null ? systemRoleFetched
                    .getStatus() : systemRole.getStatus());
            // 持久对象
            ...
            if ("NO".equals(systemRole.getStatus()))
            {
                ...
                List<SystemUserRole> relations = systemUserRoleRepository.findAll(SpecUtil
                        .spec(specs));
                if (relations != null)
                {
    ...
    public Result<Boolean> assign(Long id, Long[] functions)
    {
        ...
        if (id == null)
        {
            result = Result.newFailure("参数缺失", "分配权限时缺少必要的参数。");
            return result;
        }
        if (functions == null)
        {
            result = Result.newFailure("参数缺失", "分配权限时缺少必要的参数或者参数不完整。");
            return result;
        }
        // 删除旧记录
