...
public class SystemUserServiceImpl implements SystemUserService {
	...
	@Override
	public Result<Pager<SystemUser>> pageSystemUsers(SpecParam<SystemUser> specs, Pager<SystemUser> pager) {
		...
		// 返回结果
		PageObject<SystemUser> pageObject = new PageObject<SystemUser> ((int)resultSet.getTotalElements(), resultSet.getContent());
	...
	public Result<SystemUser> createSystemUser(SystemUser systemUser) {
	    ...
	    systemUser.setPassword(passwordService.encryptPassword(systemUser.getPassword(), systemUser.getLoginName(), systemUser.getSalt()));
		// 持久对象
	...
	public Result<SystemUser> updateSystemUser(SystemUser systemUser) {
		...
		try{
			...
			systemUserFetched.setLoginName(systemUser.getLoginName() == null ? systemUserFetched.getLoginName() : systemUser.getLoginName());
			systemUserFetched.setRealName(systemUser.getRealName() == null ? systemUserFetched.getRealName() : systemUser.getRealName());
			systemUserFetched.setMobile(systemUser.getMobile() == null ? systemUserFetched.getMobile() : systemUser.getMobile());
			systemUserFetched.setSalt(systemUser.getSalt() == null ? systemUserFetched.getSalt() : systemUser.getSalt());
			systemUserFetched.setMemo(systemUser.getMemo() == null ? systemUserFetched.getMemo() : systemUser.getMemo());
			systemUserFetched.setStatus(systemUser.getStatus() == null ? systemUserFetched.getStatus() : systemUser.getStatus());
			if(systemUser.getPassword() != null && !"".equals(systemUser.getPassword())){
				systemUserFetched.setSalt(passwordService.generateSalt());
				systemUserFetched.setPassword(passwordService.encryptPassword(systemUser.getPassword(), systemUserFetched.getLoginName(), systemUserFetched.getSalt()));
			}
			// 持久对象
			...
			if(systemUser.getRoleIds() != null){
				if(systemUser.getRoleIds().size() > 0){
					// 删除旧记录
					List<SystemUserRole>  toDeleteList = systemUserRoleRepository.findByUserId(systemUser.getId());
